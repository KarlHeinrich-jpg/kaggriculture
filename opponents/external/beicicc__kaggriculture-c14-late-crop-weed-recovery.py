...
C14-W4 adds a narrow live-state recovery for hired hands.  If an unshifted hand
is scheduled to PLANT STRAWBERRY or MELON while standing on a visible WEED
during hours 18--21, it DIGs and then follows its licensed schedule one turn
late until that in-game day ends.
Farmer and market actions are unchanged, hand state is isolated by player and
day, and no lag crosses a day boundary or the adaptive terminal.  A lag is
cancelled through a finite fixed-point check if its delayed PLANT would newly
trigger the simulator's all-or-none same-crop seed guard.  The C12 parent had
48 actor patches at 46 steps; after
the C13 ablation this version retains 43 patches at 41 steps.
"""
import base64
import copy
import json
...
_HAND_LAG_STATE = {}
_WEED_LAG_CROPS = frozenset({"STRAWBERRY", "MELON"})


def _is_live_weed(farm, position):
...
def _plant_crop(planned):
    if isinstance(planned, list) and len(planned) >= 2 and planned[0] == "PLANT":
        return planned[1]
    return None


def _apply_hand_plant_lag(action, obs):
    """DIG a hand's blocking weed, then lag only that hand through day-end."""
    player = int(obs.get("player", 0) or 0)
    ...
    if step >= 712:
        _HAND_LAG_STATE.pop(player, None)
        return action

    day = int(obs.get("day", step // 24) or 0)
    hour = int(obs.get("hour", step % 24) or 0)
    state = _HAND_LAG_STATE.get(player)

    if (
        step == 0
        or state is None
        or state["day"] != day
        or step < state["last_step"]
        or step > state["last_step"] + 1
    ):
        state = {"day": day, "last_step": step - 1, "pending": {}, "last_hands": None}
        _HAND_LAG_STATE[player] = state

    # Repeated evaluation of the same player/step must be idempotent.
    if step == state["last_step"] and state["last_hands"] is not None:
        action["hands"] = copy.deepcopy(state["last_hands"])
        return action

    hands = list(action.get("hands") or [])
    base_hands = copy.deepcopy(hands)
    positions = list(farms[player].get("hands") or [])
    pending = {
        index: value
        for index, value in state["pending"].items()
        if 0 <= index < len(hands)
    }
    lagging_indices = set(pending)

    for index, planned in enumerate(hands):
        if index in pending:
            hands[index] = copy.deepcopy(pending[index])
            pending[index] = copy.deepcopy(planned)
            continue

        op = planned[0] if isinstance(planned, list) and planned else None
        if (
            18 <= hour <= 21
            and op == "PLANT"
            and len(planned) >= 2
            and planned[1] in _WEED_LAG_CROPS
            and index < len(positions)
            and _is_live_weed(farms[player], positions[index])
        ):
            hands[index] = ["DIG"]
            pending[index] = copy.deepcopy(planned)

    # Unit actions execute before market orders.  Preserve the base action if a
    # delayed PLANT would newly make same-crop demand exceed current seeds,
    # because the simulator otherwise drops every PLANT of that crop atomically.
    farmer_action = action.get("farmer", ["PASS"])
    seeds = (obs.get("private") or {}).get("seeds") or {}
    remaining_lagging = set(lagging_indices)

    while remaining_lagging:
        base_counts = {}
        candidate_counts = {}
        for planned in [farmer_action, *base_hands]:
            crop = _plant_crop(planned)
            if crop is not None:
                base_counts[crop] = base_counts.get(crop, 0) + 1
        for planned in [farmer_action, *hands]:
            crop = _plant_crop(planned)
            if crop is not None:
                candidate_counts[crop] = candidate_counts.get(crop, 0) + 1

        blocked_crop = None
        for crop in sorted(candidate_counts):
            available = max(0, int(seeds.get(crop, 0) or 0))
            if base_counts.get(crop, 0) <= available < candidate_counts[crop]:
                blocked_crop = crop
                break
        if blocked_crop is None:
            break

        restored = []
        for index in sorted(remaining_lagging):
            if (
                index < len(hands)
                and _plant_crop(hands[index]) == blocked_crop
                and _plant_crop(base_hands[index]) != blocked_crop
            ):
                hands[index] = copy.deepcopy(base_hands[index])
                pending.pop(index, None)
                restored.append(index)

        if not restored:
            # Defensive termination: restoring every remaining lag can only
            # move the candidate action vector back to the executable base.
            for index in sorted(remaining_lagging):
                if index < len(hands):
                    hands[index] = copy.deepcopy(base_hands[index])
                    pending.pop(index, None)
            remaining_lagging.clear()
            break
        remaining_lagging.difference_update(restored)

    action["hands"] = hands
    state["day"] = day
    state["last_step"] = step
    state["pending"] = pending
    state["last_hands"] = copy.deepcopy(hands)
    return action
...
def agent(obs, config=None):
    ...
    return _apply_hand_plant_lag(action, obs)
