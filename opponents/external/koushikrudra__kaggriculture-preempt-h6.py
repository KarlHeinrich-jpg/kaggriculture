...
final = env.steps[-1]
print("self-play rewards:", [s.reward for s in final])
print("statuses:", [s.status for s in final])
assert all(s.status == "DONE" for s in final), "validation episode failed"
...
with tarfile.open("submission.tar.gz", "w:gz") as tar:
    tar.add("main.py")
print("submission.tar.gz:", os.path.getsize("submission.tar.gz"), "bytes")
print("Submit with:")
print("  kaggle competitions submit kaggriculture -f main.py -m 'presell agent'")
